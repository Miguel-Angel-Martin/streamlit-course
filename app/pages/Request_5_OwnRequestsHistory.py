...
from lib.queries import get_request_by_id_with_transaction_date_and_time_by_requester

# ----- Initialization -----
PAGE_NAME = "Request_5_OwnRequestsHistory"
...
def previous_requests_ui():
    ...
    # Consulta de requests atendidas
    requester = st.session_state.user['email']
    previous_requests_df = get_request_by_id_with_transaction_date_and_time_by_requester(requester)
    ...
    if previous_requests_df.empty:
        ...
        st.subheader("🛍️ My request history", help = (
            "From here, you can view all the requests you have made that have been attended or denied.\n\n"
        ...
        columns_to_show = ["Id", "UserRequester", "Status", "Type", "Posting_date", "Time", "Transaction_Posting_Date", "Transaction_Time", "WarehouseNotes", "Project_Id"]
        ...
        if selected_index:
            ...
            # Sumar el campo de notas de la transaccion de la request line porque es relevante en algunos casos
            transactions_df = get_transactions_notes()

            request_lines_with_material_data_df = pd.merge(
                request_lines_with_material_data_df, 
                transactions_df, 
                how='left',
                on='Transaction id', 
            )
